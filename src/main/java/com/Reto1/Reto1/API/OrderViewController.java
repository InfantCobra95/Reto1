...
public class OrderViewController {
    ...
    public ModelAndView getAll(){
        ...
        /*for(int i = 0; i < pc.products.size(); i++){
            for(int j = 0; j < orderProductInfos.size(); j++){
                if(pc.products.get(i).getName() == orderProductInfos.get(id).getNameProduct()){

                }else{
                    if(!products.contains(pc.products.get(i))){
                        products.add(pc.products.get(i));
                    }
                }
            }
        }
        ...
        mv.addObject("products", pc.products);
        

        mv.addObject("orderProduct", orderProductInfos);
        return mv;
    }
